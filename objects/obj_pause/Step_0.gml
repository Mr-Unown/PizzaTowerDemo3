if ((!pause) && (!instance_exists(obj_fadeout)))
{
    ...
    if (obj_player1.key_start && room != rank_room && room != New_Realtitlescreen && room != timesuproom)
    {
        selected = 0
        if (!instance_exists(obj_pausefadeout))
            instance_create(x, y, obj_pausefadeout)
    }
    if ((obj_player2.key_jump || keyboard_check_pressed(vk_f5)) && global.coop == 0 && room != rank_room && room != Scootertransition && room != New_Realtitlescreen && room != timesuproom) && obj_debugcontroller.active = false
    {
        global.coop = 1
        with (obj_player2)
        {
