...
for (var i = 1; i <= 2; ++i) 
{
    var player = (i == 1 ? obj_player1 : obj_player2), _grabbedby = i;
		...
		if (baddieID.state != enemystates.enemygrabbed && baddieID.state != enemystates.enemyfrozen && baddieID.state != enemystates.enemyshake && (!other.baddieID.invincible))
		{
			with (player) 
			{
				...
				if (instakillmove == 1) /*&& !(sprite_index = spr_player_shoryumineken)*/ && other.baddieID.invtime <= 0
				{
					...
					if (state == states.mach3 || (state == states.machslide && sprite_index == spr_mach3boost) || (state == states.machroll && movespeed >= 12) || state == states.freefall && freefallsmash > 10 || state == states.firemouth || state == states.superslam || state == states.finishingblow && other.baddieID.hp <= 0 || state = states.tumble || state = states.knightpepattack || state = states.knightpepslopes || state = states.knightpep)
					{
