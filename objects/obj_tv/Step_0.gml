...
if global.snickchallenge = true
    ...
    if (room == New_Realtitlescreen || room == Tutorialtrap ||  room == Titlescreen || room == rank_room || room == cowboytask || room == hub_room3 || room == hub_room2 || room == hub_room1)
        alpha = 0
    ...
    if (!(room == Titlescreen || room == Tutorialtrap || room == rank_room || room == cowboytask || room == hub_room2 || room == hub_room3  || room == hub_room1))
    {
        if collided = true
            alpha = 0.5
        else if (!(room == rank_room || room == timesuproom || room == boss_room1 || room == New_Realtitlescreen || room == Scootertransition || room == Titlescreen || room == Tutorialtrap || room == rank_room || room == cowboytask ||room == hub_room3   || room == hub_room2 || room == hub_room1))
            alpha = 1
    }
