function scr_solid_player(argument0, argument1) {
	...
	if (y > old_y && (bbox_bottom % 16) == 0 && (!place_meeting(x, old_y, obj_cottonplatform)) && place_meeting(x, y, obj_cottonplatform))
	{
	    if (state == 100 || state == 111)
	    {
	        x = old_x
	        y = old_y
	        return 1;
	...
	if (y > old_y && (bbox_bottom % 16) == 0 && (!place_meeting(x, old_y, obj_onewaywatersolid)) && place_meeting(x, y, obj_onewaywatersolid))
	{
	    if (state == 100 || state == 111)
	    {
	        x = old_x
